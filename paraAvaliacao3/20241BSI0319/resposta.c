...
typedef struct{
    char nome[64]; char simb[10];
    int numAtom; int numMassa;
    double massAtom;
} * t_ElemQuimico;
...
t_hashmap load_tabPeriodica(char *filename){
    ...
    while (fgets(buffer, 63, arq) != NULL) {
        ...
        char * token = strtok(buffer, ","); strcpy(elem->nome, token);

        token = strtok(NULL, ","); strcpy(elem->simb, token);
        token = strtok(NULL, ","); elem->numAtom = atoi(token);
        token = strtok(NULL, ","); elem->massAtom = atof(token);
        token = strtok(NULL, ","); elem->numMassa = atoi(token);

        addDado(hash, elem->simb, elem);
    }
